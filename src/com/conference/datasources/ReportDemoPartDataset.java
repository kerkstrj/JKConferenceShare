 ...
 * */
//CH - Drawing Master
public class ReportDemoPartDataset {
	...
	static 
	{
		try 
		{
			logger = LogR.getLogger(ReportDemoPartDataset.class.getName());
		} 
    ...
    public static JRBeanCollectionDataSource getDataSource(boolean useTestData)
    {
        ...
        ArrayList<ReportDemoPartRecord> reportDemoPartRecords = new ArrayList<ReportDemoPartRecord>();
        if(useTestData)
        {
        	reportDemoPartRecords.add(new ReportDemoPartRecord("8506951089","THINK V2"));
        	reportDemoPartRecords.add(new ReportDemoPartRecord("8506951090","THINK V2"));
        ...
        else
        {
        	...
        	ArrayList<HashMap<String, String>> rows = 
        				RunReportTemplate("ReportDemoPart","containerId='THINK V2'");
        	for(int i = 0;i<rows.size();i++)
        	{        		
        		reportDemoPartRecords.add(new ReportDemoPartRecord(rows.get(i).get("Persist Info.Object Identifier"),
        				rows.get(i).get("Context (containerName)"))
);
        	}
        }
        
        return new JRBeanCollectionDataSource(reportDemoPartRecords);
    }
